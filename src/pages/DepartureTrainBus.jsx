...
const BUS_COMPANIES = ['FlixBus', 'BlaBlaBus', 'Ouibus', 'ALSA', 'Eurolines', 'RegioJet', 'Flibco', 'Komfort Bus']

function generateOptions(count, minPrice, isBus) {
  const companies = isBus ? BUS_COMPANIES : TRAIN_COMPANIES
  const durationH = isBus ? 14 : 6
  const durationM = 50
  const durationLabel = isBus ? '14h 50m' : '6h 25m'
  const arrOffset = durationH * 60 + durationM
  return Array.from({ length: count }, (_, i) => {
    const depH = 6 + Math.floor(i * (14 / Math.max(count - 1, 1)))
    const depM = (i * 17) % 60
    const totalArrMin = depH * 60 + depM + arrOffset
    const arrH = Math.floor(totalArrMin / 60) % 24
    const arrM = totalArrMin % 60
    const nextDay = totalArrMin >= 1440
    const priceStep = Math.round(minPrice * 0.06 / 1000) * 1000
    const price = minPrice + i * priceStep
    const fmt = (n) => String(n).padStart(2, '0')
    return { id: i+1, dep: `${fmt(depH)}:${fmt(depM)}`, arr: `${fmt(arrH)}:${fmt(arrM)}${nextDay?'+1':''}`, company: companies[i % companies.length], price, duration: durationLabel, isLowest: i===0 }
  })
}

const TAG_STYLES = { '직항': 'bg-[#e6f5e8] text-[#008026]', '최저가': 'bg-[#fff4c2] text-[#c24c00]' }
...
export default function DepartureTrainBus() {
  ...
  const options  = generateOptions(count, minPrice, isBus)
        ...
        <div className="flex gap-[10px] overflow-x-auto px-[16px] py-[10px] bg-white border-b border-[#f0f0f0]">
          {filters.map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`h-[28px] px-[12px] rounded-full whitespace-nowrap flex-shrink-0 text-[12px] font-medium ${filter===f?'bg-[#132968] text-white':'border border-[#e5e7ee] text-[#6b7281]'}`}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-[16px] py-[16px] flex flex-col gap-[16px]">
        {options.map((o) => (
          <button key={o.id} onClick={() => setSelected(o.id)}
            className={`bg-white border-2 rounded-[8px] py-[12px] flex flex-col gap-[8px] w-full transition-colors ${selected===o.id?'border-[#fa6b6b]':'border-[#ccc]'}`}>
